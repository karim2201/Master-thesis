...
\subsection{Physics-Informed Real-to-Simulation Extension}
A future CardioTwin version could extend the current patient-state prototype toward physics-informed real-to-simulation modeling. Such an extension would require integrating echocardiography or another non-invasive cardiovascular imaging modality, implementing a Windkessel or reduced-order hemodynamic model, generating synthetic parameter--state samples, training a differentiable forward surrogate, and inferring patient-specific parameters from non-invasive observations. The reconstructed states could include pressure, flow, volume, or pressure--volume behavior, and the resulting model could support controlled intervention scenarios.

This direction remains future research. It would require validation of inferred states against independent clinical measurements, explicit uncertainty quantification, identifiability analysis, careful definition of parameter ranges, and prospective clinical evaluation before any individual therapeutic interpretation. It must not be presented as a capability of the current academic prototype.
