\chapter{Selected Architecture for a Digital Twin-Based eHealth System}
...
For physiological patient twins, this layer may optionally implement a hybrid real-to-simulation pattern inspired by recent physics-informed Digital Twin methods. Such a pattern can include a non-invasive data encoder, an inverse patient-parameter estimator, a mechanistic physiological model, a differentiable or reduced-order surrogate, an intervention model, a physiological-plausibility checker, and an uncertainty-estimation component. The workflow is: non-invasive observations are processed to estimate latent patient parameters; the mechanistic model simulates physiological states; reconstructed observable variables are compared with available measurements; plausibility and uncertainty are checked; and controlled what-if services are generated only within the validated domain of the model.

This pattern does not replace the general architecture and is not required for every eHealth application. It is one possible implementation of the intelligence and simulation layer for physiological twins. The system must explicitly distinguish measured inputs, model-estimated parameters, simulated physiological states, predicted future values, and counterfactual intervention outputs.
...
\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/fig_data_flow}}
...
The selected architecture can be evaluated using several criteria:
...
\section{Summary}
This chapter presented the selected general conceptual architecture for Digital Twin-based eHealth systems. The architecture is designed to be reusable, modular, secure, interoperable, and adaptable to multiple healthcare scenarios.
