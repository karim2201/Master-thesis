\chapter{State-of-the-Art Review: Digital Twins in eHealth and Cardiovascular Medicine}
...
Med-Real2Sim by Kuang et al.~\cite{kuang2024medreal2sim} is an important example of AI used as an inverse-identification component inside a Digital Twin rather than as a direct clinical classifier. Its general pipeline maps a non-invasive medical image to patient-specific physiological parameters, passes those parameters through a mechanistic simulator, and reconstructs simulated physiological states. This differs from a conventional convolutional model that directly predicts an outcome such as a label or ejection fraction. In Med-Real2Sim, AI estimates the parameters required to initialize a patient-specific cardiovascular simulator; the physiological model constrains the learned representation, and synthetic simulator data transfer physical structure into the learning process. Because the complete analysis is cardiovascular, the study is treated in detail in Section~\ref{sec:medreal2sim}.
...
% =========================================================
\section{Cardiovascular Digital Twins}
...
\subsection[Non-Invasive Hemodynamic Digital Twins through Physics-Informed Self-Supervised Learning]{Non-Invasive Hemodynamic Digital Twins through\\ Physics-Informed Self-Supervised Learning}
\label{sec:medreal2sim}
Med-Real2Sim, proposed by Kuang et al.~\cite{kuang2024medreal2sim}, provides a methodological bridge between patient-specific cardiovascular simulation and non-invasive Digital Twin construction. The clinical motivation is clear: pressure--volume loops describe the relation between ventricular pressure and ventricular volume during the cardiac cycle and can inform contractility, ventricular filling, stroke work, loading conditions, hemodynamic response, and possible intervention effects. However, complete pressure--volume characterization often requires invasive catheter-based measurements. Echocardiography can provide non-invasive information about cardiac motion, ventricular geometry, end-diastolic volume, end-systolic volume, and ejection fraction, but it does not directly measure the complete ventricular-pressure trajectory.

The scientific problem addressed by the study is therefore not simply ejection-fraction prediction. It is the inverse problem of estimating patient-specific parameters of a mechanistic cardiovascular model from non-invasive echocardiography and then using the model to reconstruct physiological states that are not directly observed. In simplified notation, the mechanistic model maps a parameter vector to physiological states:
\[
\mathbf{x}=M(\boldsymbol{\theta}),
\]
where \(M\) is the cardiovascular model, \(\boldsymbol{\theta}\) is the patient-specific physiological parameter vector, and \(\mathbf{x}\) represents pressure, volume, flow, and related states. A medical observation is produced through an observation process:
\[
\mathbf{y}=K(\mathbf{x})=K(M(\boldsymbol{\theta})),
\]
where \(\mathbf{y}\) is the non-invasive observation and \(K\) represents the partly unknown relation between internal physiology and the appearance of the medical image. In the Med-Real2Sim application, \(\mathbf{y}\) is an echocardiography video. The inverse chain is therefore echocardiography, estimated patient parameters, patient-specific simulator, and simulated cardiovascular states.

This formulation is composite and difficult. The mechanistic cardiovascular model is available, but the image-formation and observation process is not fully modeled. Direct labels for hidden patient-specific parameters such as valve resistance or ventricular elastance are not available in the public datasets used by the authors. Moreover, several parameter combinations may reproduce similar observable volumes or ejection fractions. The method should therefore be interpreted as a physics-informed identification framework, not as proof that all inferred hidden parameters are uniquely or invasively validated.

The learning strategy has two stages. First, the authors define physiologically plausible parameter ranges, sample parameter combinations, run the mechanistic cardiovascular simulator, and generate synthetic physiological states. A neural network is then trained to approximate the forward simulator. This network acts as a differentiable surrogate that embeds knowledge from the mechanistic model into a trainable component. The synthetic examples are simulator-generated parameter--state pairs, not real patient measurements. Second, the authors process real echocardiography videos with a three-dimensional convolutional neural network that estimates patient-specific parameters. These estimated parameters are passed through the frozen physics surrogate, which reconstructs observable cardiac quantities such as end-diastolic and end-systolic ventricular volumes. The image-to-parameter network is updated by comparing reconstructed observable quantities with available clinical labels. Thus, the model is supervised through observable cardiac quantities, but the hidden physiological parameters remain indirectly identified. A close match in ventricular volume or ejection fraction does not necessarily prove that every inferred resistance, elastance, pressure, or hidden state is physiologically unique or correct.

The mechanistic component is based on a Windkessel-style lumped-parameter cardiovascular model. A Windkessel model uses an electrical-circuit analogy to describe cardiovascular behavior: pressure corresponds to voltage, blood flow to electrical current, vascular or valve resistance to resistance, compliance to capacitance, flow inertia to inductance, and one-directional cardiac valves to diodes. Kuang et al. use a reduced circuit model with compartments including the left ventricle, left atrium, arterial and aortic components, valves, and connected flow pathways. Ordinary differential equations describe the evolution of pressure, volume, and flow. The image network estimates seven parameters: mitral-valve resistance, aortic-valve resistance, maximum ventricular elastance, minimum ventricular elastance, theoretical left-ventricular volume at zero pressure, initial left-ventricular volume, and cardiac-cycle duration. These are model-estimated latent parameters, not direct clinical measurements.

The experiments use two echocardiography datasets. CAMUS contains 500 annotated cardiac ultrasound videos in two-chamber view with left-ventricular end-systolic and end-diastolic volume labels. EchoNet-Dynamic contains 10,030 apical four-chamber echocardiography videos from routine clinical care with ventricular-volume and ejection-fraction information. Kuang et al. report ejection-fraction mean absolute errors of 6.81 percentage points on CAMUS and 5.40 percentage points on EchoNet-Dynamic for Med-Real2Sim, compared with 6.58 and 5.62 percentage points for a direct supervised 3D-CNN baseline. The important result is not merely that ejection fraction is predicted competitively, but that the model also produces patient-specific latent parameters, simulated pressure--volume loops, and a mechanistic basis for counterfactual simulation.

The disease-analysis experiment is exploratory. The authors obtained mitral-stenosis labels for a subset of EchoNet-Dynamic and state that these labels were not included in training. They report 263 patients with mitral stenosis among 10,024 available patients and compare average simulated pressure--volume loops for 100 patients with mitral stenosis and 100 without. The analysis suggests that the inferred simulated states may contain hemodynamic information not directly encoded in the ejection-fraction training target. However, it should be classified as an exploratory disease-related analysis, not as validated autonomous mitral-stenosis diagnosis. It is group-level, uses a limited labeled subset, and lacks direct invasive validation of the inferred pressure states.

The same framework is also used for counterfactual intervention simulation. The authors extend the lumped-parameter model with a left-ventricular assist device representation, allowing the same patient-specific simulator to be evaluated under baseline conditions and hypothetical LVAD support. They report average simulated ejection-fraction increases of 17.6\% for the CAMUS simulated population and 18.9\% for the EchoNet-Dynamic simulated population after adding the LVAD model. These are model-generated counterfactual simulation results, not observed prospective clinical outcomes. Their relevance to Digital Twin services is that an identified physiological simulator can be perturbed under controlled assumptions; their limitation is that intervention predictions require independent validation before any individual treatment interpretation.

Figure~\ref{fig:med_real2sim_pipeline} summarizes the real-to-simulation construction as an author synthesis. The upper path represents physics pretraining from plausible parameter ranges and simulator-generated states. The lower path represents patient identification from echocardiography to estimated parameters, simulated physiological states, and pressure--volume outputs. The intervention branch indicates how hypothetical device or treatment parameters can be introduced into the simulator.

\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/fig_med_real2sim_pipeline}}
    \caption{Physics-informed real-to-simulation construction of a non-invasive cardiovascular Digital Twin. Author synthesis based on Kuang et al.~\cite{kuang2024medreal2sim}.}
    \label{fig:med_real2sim_pipeline}
\end{figure}

Compared with direct supervised prediction, Med-Real2Sim is more physiologically structured because it produces latent parameters and simulator states rather than only a label. Compared with conventional mechanistic simulation, it reduces the dependence on paired invasive parameter measurements by learning an image-to-parameter mapping from non-invasive data and observable volume labels. Compared with traditional PINNs and Neural ODE approaches, its surrogate is trained once at population level over plausible parameter ranges, while patient-specific parameters are inferred through the image encoder. The method is therefore best classified as a retrospective patient-specific hemodynamic simulation framework or partial Digital Twin based on episodic non-invasive imaging. It is not a continuously synchronized clinical twin, a replacement for invasive pressure measurement, or a validated LVAD recommendation platform.
...
\addlinespace
Kuang et al.~\cite{kuang2024medreal2sim} -- Med-Real2Sim & Non-invasive identification of patient-specific cardiac hemodynamic models and pressure--volume simulation & Echocardiography videos with ventricular-volume and ejection-fraction information from CAMUS and EchoNet-Dynamic & Lumped-parameter Windkessel cardiovascular model, synthetic simulator data, differentiable physics surrogate, 3D-CNN inverse model, and physics-informed self-supervised learning & Retrospective patient-specific hemodynamic simulation / partial Digital Twin & CAMUS and EchoNet-Dynamic evaluation; competitive EF prediction; estimated patient-specific physiological parameters; simulated pressure--volume loops; exploratory mitral-stenosis analysis; counterfactual LVAD simulation & Hidden parameters not directly validated using invasive measurements; identifiability uncertainty; model-dependent assumptions; limited uncertainty quantification; episodic imaging rather than longitudinal synchronization; retrospective datasets; intervention simulation not clinically validated \\
\addlinespace
...
Med-Real2Sim occupies an important intermediate position between offline mechanistic cardiovascular simulation and continuously synchronized clinical Digital Twins. Its main innovation is not a new clinical endpoint but the non-invasive identification of a patient-specific mechanistic simulator. The study strengthens the case for hybrid cardiovascular twins that combine mechanistic interpretability, learned inverse parameter estimation, synthetic simulator data, reusable population-level models, patient-specific simulation, and counterfactual services. At the same time, it exposes unresolved issues that remain central to cardiovascular Digital Twin maturity: hidden-parameter identifiability, model misspecification, uncertainty propagation, domain shift across imaging settings, absence of invasive ground truth for inferred pressure states, lack of continuous updating, and limited clinical workflow validation.
...
\subsubsection{Methodological Lessons from Med-Real2Sim}
Med-Real2Sim provides several lessons for CardioTwin even though it is not part of the current prototype implementation. First, measured variables must be separated from estimated latent variables and simulated states. Second, AI should be constrained by physiological models when the output is intended to support patient-specific simulation. Third, synthetic simulator data can transfer physiological structure into a learning system, but it cannot replace external and clinical validation. Fourth, non-invasive inputs can be used to estimate patient-specific parameters only when plausible parameter ranges, observable reconstruction targets, and validation procedures are clearly defined. Fifth, personalized models are useful for controlled what-if analysis only when uncertainty, assumptions, and model limits are reported.

In the present PFE scope, CardioTwin focuses on ECG, SBP, DBP, heart rate when available, demographic and clinical information, AI-assisted ECG analysis, patient-state storage, cardiovascular risk assessment, and evidence-based what-if scenarios. It does not include echocardiography-video analysis, patient-specific pressure--volume loop reconstruction, invasive left-ventricular pressure validation, clinically validated Windkessel parameter estimation, LVAD simulation, or verified individual hemodynamic intervention prediction. Med-Real2Sim therefore provides a methodological roadmap for a future physics-informed CardioTwin extension rather than a description of a capability already implemented in the current PFE.
...
\addlinespace
Non-invasive observations can constrain latent physiological parameter estimation & Move beyond direct classification toward patient-state identification & Future inverse model constrained by a reduced cardiovascular simulator & No validated hemodynamic parameter ground truth in the current prototype\\
\addlinespace
Synthetic simulator data can transfer physiological structure to learning models & Reduce dependence on paired invasive labels while preserving model constraints & Future Windkessel or reduced-order simulation extension & Synthetic data cannot replace external and clinical validation\\
\addlinespace
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Definitions and benchmarks:} Digital Model, Digital Shadow, partial connected Digital Twin, process Digital Twin, and intelligent Digital Twin are not always distinguished consistently despite the definition framework discussed in Chapter~\ref{chap:dt_fundamentals}.
...
AI increases the analytical power of Digital Twins but also creates specific gaps. Complex models require explainability, calibration, subgroup analysis, bias assessment, uncertainty estimation, and out-of-distribution testing. Adaptive models require drift detection and documented revalidation. Generative AI and LLM-based components require governance because synthetic outputs may reproduce bias, leak private information, or appear more clinically reliable than they are. A major methodological gap is the limited comparison between pure AI systems, pure mechanistic simulations, and hybrid approaches under the same clinical objectives. Another gap is the risk of interpreting overfitting as personalization when the model has not been externally validated. Hybrid real-to-simulation approaches add further gaps: weakly supervised physiological parameter estimation, non-identifiable latent variables, missing predictive uncertainty, absent calibration intervals, domain shift across imaging devices and institutions, uncertainty propagated from mechanistic models into learned surrogates, and difficulty distinguishing clinically meaningful personalization from overfitting.

\subsection{Cardiovascular Digital Twin Gaps}
The cardiovascular literature adds domain-specific challenges. High-fidelity models often depend on specialized imaging, invasive measurements, patient-specific segmentation, boundary-condition estimation, and expensive computation. Physiological parameters may be difficult to identify uniquely, and several parameter sets can explain the same ECG, pressure, or flow observations. Agreement with observable variables such as ventricular volume or ejection fraction does not necessarily demonstrate that the inferred hidden physiological parameters are uniquely correct. Longitudinal synchronization remains uncommon, multicenter validation is still limited, and many studies treat ECG, imaging, pressure, flow, and clinical history separately rather than as a continuously updated patient-state representation. Additional gaps include scarcity of paired non-invasive and invasive datasets, difficulty validating inferred pressure and flow states, ill-posed inverse cardiovascular problems, dependence on parameter ranges and prior assumptions, and limited prospective validation of intervention simulations. What-if treatment simulation remains scientifically promising but clinically sensitive because predicted medication or intervention effects require prospective validation before individual therapeutic use.
